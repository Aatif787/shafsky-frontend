...
export default function BookingView({ searchParams }: BookingViewProps) {
  ...
  const maxSteps = 3;
  const currentConfig = charterStepConfigs[Math.min(currentStep - 1, charterStepConfigs.length - 1)];

  return (
    <div className="min-h-screen bg-[#FAF9F5] text-slate-900 py-8 sm:py-12 px-4 sm:px-8 max-w-5xl mx-auto">
      {/* 1. SHARED PROGRESS HEADER */}
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <select
                        value={charter.aircraftCategory}
                        onChange={(e) => charter.setAircraftCategory(e.target.value)}
                        className="w-full px-4 py-3.5 rounded-2xl bg-white border border-slate-200 text-slate-900 text-sm focus:outline-none focus:border-emerald-500 shadow-xs font-sans font-medium"
                      >
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  ...
                  {/* 3. SHARED REVIEW SUMMARY */}
                  <ReviewSummary
                    serviceTitle={charter.aircraftCategory}
                      ...
                      try {
                        ...
                        await submitBookingFn({
                          data: {
                            ...
                            special_requests: charter.aircraftCategory,
              ...
              {/* 4. SHARED SUCCESS PASS */}
              {currentStep === 3 && (
                <BookingSuccessPass
                  badge="Charter Quote Staged"
                  title="Private Jet Quote Request Logged"
                  subtitle={`Our flight dispatch desk is staging tail options for ${pickupCity} → ${destinationCity} (${charter.aircraftCategory}).`}
...
print("Executed Workflow State Migration for BookingView.tsx.")
