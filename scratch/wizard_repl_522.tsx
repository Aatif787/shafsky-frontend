  ...
  const getAirportDisplayName = useCallback((code: string) => {
  ...
  const resolvedServices = useMemo(() => {
    ...
    return rawServices.map((svc) => {
      ...
      const match = allServices.find((s) => s.id === allServices.find(as => as.id === localId) ? localId : "");
      const matchItem = allServices.find((s) => s.id === localId);
      return {
        ...svc,
        title: matchItem?.title || svc.title,
        desc: matchItem?.desc || svc.desc,
        price: matchItem?.price || svc.price,
      };
    });
