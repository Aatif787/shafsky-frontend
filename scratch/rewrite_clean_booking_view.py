...
import { PassengerInformationExperience } from "@/components/booking/PassengerInformationExperience";
import { BOOKING_CONFIGURATIONS, getServiceBookingConfig, type ServiceBookingConfig } from "@/data/bookingConfigurations";
import { getWorkflowConfig, WORKFLOW_REGISTRY, type WorkflowConfig } from "@/data/workflowRegistry";
...
export default function BookingView({ searchParams }: BookingViewProps) {
  ...
  // Meet & Greet Direct Flow Detection
  const [flightDirection, setFlightDirection] = useState<"arrival" | "departure">("arrival");
  const isMeetGreetWorkflow =
    searchParams?.service_id === "meet_greet" ||
    searchParams?.package_id ||
    (!searchParams?.service_id && true); // Meet & Greet is flagship default

  const [serviceCategory, setServiceCategory] = useState<string>("concierge");
  const [subServiceId, setSubServiceId] = useState<string>(searchParams?.service_id || "meet_greet");
  ...
  const [flightDuration, setFlightDuration] = useState<string>("");
  const [aircraftModel, setAircraftModel] = useState<string>("");

  const [flightSearchState, setFlightSearchState] = useState<
    "idle" | "searching" | "found" | "failed" | "manual"
  >("idle");
  const [autocompleteSuggestion, setAutocompleteSuggestion] = useState<string | null>(null);
  const [flightData, setFlightData] = useState<FlightDataResult | null>(null);
  const [editingFlightResult, setEditingFlightResult] = useState(false);
  const flightInputRef = useRef<HTMLInputElement>(null);

  const [selectedAddons, setSelectedAddons] = useState<string[]>(["lounge", "fast_track"]);
  ...
  const handleFlightNumberChange = useCallback((rawValue: string) => {
    const upper = rawValue.toUpperCase().replace(/\\s+/g, "");
    setFlightNumber(upper);
    setFlightSearchState("idle");
    setFlightData(null);
    setEditingFlightResult(false);
    const guessed = guessAirline(upper);
    setAutocompleteSuggestion(guessed);
  }, []);

  const handleFlightSearch = useCallback(async () => {
    if (!flightNumber || flightNumber.length < 3) {
      toast.error("Please enter a valid flight number (e.g. AI302, EK511).");
      return;
    }

    setFlightSearchState("searching");
    setFlightData(null);

    try {
      const res = await fetch("/api/flight/validate", {
        method: "POST",
      ...
      if (json.success && json.data) {
        const flight: FlightDataResult = Array.isArray(json.data) ? json.data[0] : json.data;
        setFlightData(flight);
        setAirline(flight.carrier?.name || "");
        setOriginAirport(flight.origin?.code || originAirport);
        setDestAirport(flight.destination?.code || destAirport);
        setDepartureTerminal(flight.departure?.terminal || "");
        setArrivalTerminal(flight.arrival?.terminal || "");
        setFlightStatus(flight.status || "Scheduled");
        setFlightDuration(flight.duration || "");
        setAircraftModel(flight.aircraft?.model || "");

        if (flight.departure?.scheduledTime) {
          try {
            const dt = new Date(flight.departure.scheduledTime);
            if (!isNaN(dt.getTime())) {
              const hrs = String(dt.getHours()).padStart(2, "0");
              const mins = String(dt.getMinutes()).padStart(2, "0");
              setFlightTime(`${hrs}:${mins}`);
            }
          } catch {
            // keep existing flightTime
          }
        }

        setFlightSearchState("found");
        setAutocompleteSuggestion(null);
        toast.success("Flight verified! Details populated automatically.");
      } else {
        setFlightSearchState("failed");
      }
    } catch {
      setFlightSearchState("failed");
    }
  }, [flightNumber, flightDate, originAirport, destAirport]);

  const getBasePrice = () => {
    let base = 12500;
    if (subServiceId === "lounge") base = 9500;
  ...
  const getAddonsPrice = () => {
    const airportServices = getAirportServices(originAirport);
    let total = 0;
    for (const addonId of selectedAddons) {
      const svc = airportServices.find((s) => s.id === addonId);
      if (svc) {
        if (svc.price) {
          total += svc.price;
        } else if (svc.packages && svc.packages.length > 0) {
          total += svc.packages[0].price;
        }
      }
    }
    return total * paxAdults;
  };

  const totalPrice = getBasePrice() + getAddonsPrice();
  ...
  const stepConfigs = [
    {
      ...
      sub: "Choose your primary airside concierge or aviation service.",
      estTime: "Est. 30 sec",
      progress: 14,
    },
    {
      title: "Flight & Travel Information",
      sub: "Search your flight number and we'll auto-fill the rest.",
      estTime: "Est. 45 sec",
      progress: 28,
    },
    {
      title: "Staged Airport Add-Ons",
      sub: "Select verified services staged for your airport hub.",
      estTime: "Est. 45 sec",
      progress: 42,
    },
    {
      title: "Passenger & Guest Details",
      sub: "Enter lead guest contact information and headcount.",
      estTime: "Est. 45 sec",
      progress: 57,
    },
    {
      title: "Review Booking Itinerary",
      sub: "Verify your complete itemized service breakdown before payment.",
      estTime: "Est. 30 sec",
      progress: 71,
    },
    {
      title: "Payment & Deposit Selection",
      sub: "Choose your preferred secure payment method or corporate invoice.",
      estTime: "Est. 30 sec",
      progress: 85,
    },
    {
      title: "Booking Confirmed & Digital Pass",
      sub: "Your airside concierge pass is active and assigned to our 24/7 command desk.",
      estTime: "Completed",
      progress: 100,
    },
  ];

  const activeConfigs = isMeetGreetWorkflow ? meetGreetStepConfigs : stepConfigs;
  const maxSteps = isMeetGreetWorkflow ? 3 : 7;
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {isMeetGreetWorkflow ? (
            ...
            /* FALLBACK MULTI-STEP WORKFLOW */
            <div className="space-y-6">
              {currentStep === 1 && (
                <div className="space-y-6">
                  <p className="text-xs sm:text-sm text-slate-600 font-sans">{currentConfig.sub}</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.values(BOOKING_CONFIGURATIONS).map((s) => {
                      const Icon = s.icon;
                      const isSelected = subServiceId === s.id;
                      return (
                        <div
                          key={s.id}
                          onClick={() => setSubServiceId(s.id)}
                          className={`p-6 rounded-3xl border transition-all duration-300 cursor-pointer flex flex-col justify-between ${
                            isSelected ? "bg-emerald-50 border-emerald-500 shadow-xs" : "bg-white border-slate-200"
                          }`}
                        >
                          <div>
                            ...
                            <h3 className="text-lg font-serif font-bold text-slate-900">{s.serviceName}</h3>
                            <p className="mt-1.5 text-xs text-slate-600 font-sans">{s.tagline}</p>
                          </div>
                          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] font-mono uppercase tracking-widest text-emerald-700 font-bold">
...
print("Rewrote BookingView.tsx cleanly.")
