  ...
  return (
    <div className="relative space-y-1.5" ref={containerRef}>
      ...
      <button
        ...
        className="w-full px-4 py-2.5 bg-slate-950/70 border border-slate-800 hover:border-slate-700 focus:border-amber-500 rounded-xl text-sm text-left flex items-center justify-between transition-all focus:outline-none focus:ring-1 focus:ring-amber-500/40"
      >
        <div className="flex items-center gap-2.5 overflow-hidden">
          {selectedCountry ? (
            <>
              <span className="text-lg leading-none">{selectedCountry.flag}</span>
              <span className="font-semibold text-white truncate">{selectedCountry.name}</span>
            </>
          ) : value ? (
            <>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden animate-in fade-in duration-150 max-h-64 flex flex-col">
          {/* Search Bar */}
          <div className="p-2 border-b border-slate-800 bg-slate-950/60 sticky top-0">
